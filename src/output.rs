...
fn strip_footer(path: &Path) -> Result<()> {
    ...
    let pos = match find_footer_position(path, file_size, STRIP_FOOTER_TAIL_WINDOW)? {
        Some(p) => Some(p),
        None => find_footer_position(path, file_size, STRIP_FOOTER_TAIL_WINDOW_LARGE)?,
    };
    ...
    truncate_file_atomic(path, pos)
}

fn find_footer_position(path: &Path, file_size: u64, window: u64) -> Result<Option<u64>> {
    if file_size == 0 {
        return Ok(None);
    }
...
fn truncate_file_atomic(path: &Path, pos: u64) -> Result<()> {
    let parent = path
        .parent()
