...
// ============================================================================
// SessionModel projections (C7 model-backed output layer)
// ============================================================================

use crate::timeline::FrameKind;
use aicx_parser::engine::{Known, SessionModel, TurnKind, TurnRole};
...
pub fn frame_kind_for_turn(kind: TurnKind) -> FrameKind {
    match kind {
        ...
        TurnKind::ToolCall | TurnKind::ToolResult => FrameKind::ToolCall,
        TurnKind::SystemNote => FrameKind::SystemNote,
    }
}

/// Stable role string for a typed turn role.
pub fn role_str_for_turn(role: TurnRole) -> &'static str {
    match role {
        TurnRole::User => "user",
...
fn known_str(value: &Known<String>) -> Option<&str> {
    match value {
...
fn parse_model_timestamp(value: &Known<String>) -> Option<DateTime<Utc>> {
    known_str(value)
        .and_then(|raw| DateTime::parse_from_rfc3339(raw).ok())
        .map(|parsed| parsed.with_timezone(&Utc))
}

/// Project a validated `SessionModel` into report timeline entries.
///
/// Pure function of the model: no filesystem access, no re-parse of the raw
/// source, no store reads. Unknown turn timestamps fall back to the session's
/// provenance `started_at` (marked via `timestamp_source`) instead of
/// fabricating per-turn times.
pub fn timeline_entries_from_model(model: &SessionModel) -> Vec<TimelineEntry> {
    let agent = model.provenance.agent.as_str().to_string();
    let session_start = parse_model_timestamp(&model.provenance.started_at);
    model
        .turns
        .iter()
        .map(|turn| {
            ...
            let turn_timestamp = parse_model_timestamp(&turn.timestamp);
            let (timestamp, timestamp_source) = match (turn_timestamp, session_start) {
                (Some(timestamp), _) => (timestamp, None),
                (None, Some(start)) => (start, Some("session_provenance".to_string())),
                (None, None) => (DateTime::<Utc>::UNIX_EPOCH, Some("unknown".to_string())),
            };
            let line_span = turn
                .raw_unit_refs
                .iter()
                .map(|unit| unit.physical_ordinal)
                .fold(None::<(u64, u64)>, |span, ordinal| match span {
                    Some((low, high)) => Some((low.min(ordinal), high.max(ordinal))),
            ...
            TimelineEntry {
                ...
                source_path: None,
                source_sha256: Some(model.provenance.original_source_hash.clone()),
                source_line_span: line_span,
            }
        })
        .collect()
}
