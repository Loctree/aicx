...
// ── AICX_SOURCE_HOME env-override tests ──────────────────────────────────────
//
// `AICX_SOURCE_HOME` is a process-global env var; parallel cargo test threads
// race on set/unset without coordination. The Mutex pattern (mirroring
// `store::tests::AICX_HOME_ENV_LOCK`) serializes every test that touches it.
// The RAII guard restores the previous value so a panicking test cannot leak
// state into siblings.

static AICX_SOURCE_HOME_ENV_LOCK: std::sync::Mutex<()> = std::sync::Mutex::new(());

struct SourceHomeEnvGuard {
    prev: Option<std::ffi::OsString>,
}

impl SourceHomeEnvGuard {
    fn capture() -> Self {
        Self {
            prev: std::env::var_os("AICX_SOURCE_HOME"),
        }
    }
}

impl Drop for SourceHomeEnvGuard {
    fn drop(&mut self) {
        // SAFETY: lock is held by AICX_SOURCE_HOME_ENV_LOCK Mutex; no
        // concurrent env access for this var during the test+restore window.
        match self.prev.take() {
            Some(value) => unsafe { std::env::set_var("AICX_SOURCE_HOME", value) },
            None => unsafe { std::env::remove_var("AICX_SOURCE_HOME") },
        }
    }
}

#[test]
fn resolve_source_home_honors_explicit_env_var() {
    let _serial = AICX_SOURCE_HOME_ENV_LOCK
        .lock()
        .unwrap_or_else(|p| p.into_inner());
    let _guard = SourceHomeEnvGuard::capture();
    unsafe { std::env::set_var("AICX_SOURCE_HOME", "/tmp/test-source-home-explicit") };
    let resolved = resolve_source_home().expect("should resolve");
    assert_eq!(resolved, PathBuf::from("/tmp/test-source-home-explicit"));
}

#[test]
fn resolve_source_home_falls_back_to_home_dir_when_env_unset() {
    let _serial = AICX_SOURCE_HOME_ENV_LOCK
        .lock()
        .unwrap_or_else(|p| p.into_inner());
    let _guard = SourceHomeEnvGuard::capture();
    unsafe { std::env::remove_var("AICX_SOURCE_HOME") };
    let resolved = resolve_source_home().expect("should resolve");
    // Must be non-empty and not the literal override path from the
    // explicit-env test above.
    assert!(!resolved.as_os_str().is_empty());
    assert_ne!(resolved, PathBuf::from("/tmp/test-source-home-explicit"));
}

#[test]
fn resolve_source_home_treats_empty_env_as_unset() {
    let _serial = AICX_SOURCE_HOME_ENV_LOCK
        .lock()
        .unwrap_or_else(|p| p.into_inner());
    let _guard = SourceHomeEnvGuard::capture();
    unsafe { std::env::set_var("AICX_SOURCE_HOME", "") };
    let resolved = resolve_source_home().expect("should resolve");
    assert!(!resolved.as_os_str().is_empty());
}

#[test]
fn extract_claude_with_source_home_override_returns_empty_for_missing_dir() {
    let _serial = AICX_SOURCE_HOME_ENV_LOCK
        .lock()
        .unwrap_or_else(|p| p.into_inner());
    let _guard = SourceHomeEnvGuard::capture();
    let tmp = unique_test_dir("source-home-claude");
    unsafe { std::env::set_var("AICX_SOURCE_HOME", tmp.display().to_string()) };
    let result = extract_claude(&default_config(false));
    assert!(result.is_ok());
    assert!(result.unwrap().is_empty());
}

#[test]
fn extract_codex_sessions_with_source_home_override_returns_empty_for_missing_dir() {
    let _serial = AICX_SOURCE_HOME_ENV_LOCK
        .lock()
        .unwrap_or_else(|p| p.into_inner());
    let _guard = SourceHomeEnvGuard::capture();
    let tmp = unique_test_dir("source-home-codex-sess");
    unsafe { std::env::set_var("AICX_SOURCE_HOME", tmp.display().to_string()) };
    let result = extract_codex_sessions(&default_config(false));
    assert!(result.is_ok());
    assert!(result.unwrap().is_empty());
}
