...
fn source_info(
    ...
    SourceInfo {
        ...
        git_remotes,
        protection_warning: (!protected_by_git)
            .then(|| super::UNPROTECTED_SOURCE_WARNING.to_string()),
    }
}
...
pub fn list_available_sources() -> Result<Vec<SourceInfo>> {
    ...
    if codex_sessions_dir.exists() && codex_sessions_dir.is_dir() {
        let files = super::walk_jsonl_files(&codex_sessions_dir);
    ...
    if gemini_tmp.exists() && gemini_tmp.is_dir() {
        for project_entry in fs::read_dir(&gemini_tmp)? {
            ...
            for file_entry in fs::read_dir(&chats_dir)? {
                let file_entry = file_entry?;
                let fp = file_entry.path();
                if super::is_gemini_session_file(&fp) {
                    session_count += 1;
                    if let Ok(meta) = fs::metadata(&fp) {
                        total_size += meta.len();
                    }
    ...
    if junie_sessions.exists() && junie_sessions.is_dir() {
        let files: Vec<PathBuf> = super::walk_jsonl_files(&junie_sessions)
            .into_iter()
            .filter(|path| {
                path.file_name().and_then(|name| name.to_str())
                    == Some(super::JUNIE_EVENTS_FILENAME)
            })
            .collect();
        let total_size: u64 = files
            .iter()
    ...
    let codescribe_transcripts = super::discover_codescribe_transcripts(&home);
    ...
    let operator_markdown = super::discover_operator_markdown(&home);
...
fn count_codex_sessions(path: &Path) -> Result<usize> {
    ...
    while let Some(limited) = sanitize::read_line_capped(&mut reader, super::MAX_LINE_BYTES)? {
        line_number += 1;
        if limited.exceeded {
            super::observe_oversized_line(
                &mut oversized_count,
                &mut oversized_samples,
                line_number,
            );
            continue;
        }
        ...
        if let Ok(entry) = serde_json::from_str::<super::CodexEntry>(&line) {
            sessions.insert(entry.session_id);
        }
    }

    if oversized_count > 0 {
        super::emit_codex_session_warnings(
            path,
            &[super::CodexSessionWarning::OversizedLine {
