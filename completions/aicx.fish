...
function __fish_aicx_global_optspecs
    string join \n v/verbose project-fuzzy help-full h/help V/version
end
...
complete -c aicx -n "__fish_aicx_needs_command" -l help-full -d 'Show every command, including the power-user surfaces hidden from the short help (catalog internals, per-agent extractors, intents, migrations, archive readers)'
...
complete -c aicx -n "__fish_aicx_needs_command" -f -a "overlay" -d 'Join catalog typed intents to the current Loctree anchor catalog'
...
complete -c aicx -n "__fish_aicx_needs_command" -f -a "extract" -d 'Extract sessions — one at a time, or every compatible source at once'
complete -c aicx -n "__fish_aicx_needs_command" -f -a "conversations" -d 'Batch-export conversation JSON files without writing per-frame card files'
...
complete -c aicx -n "__fish_aicx_needs_command" -f -a "store" -d 'Rebuild the durable extract-era session catalog (no per-frame cards)'
...
complete -c aicx -n "__fish_aicx_needs_command" -f -a "wizard" -d 'Interactive daily-driver entrypoint for catalog, extracts, index, and doctor'
complete -c aicx -n "__fish_aicx_needs_command" -f -a "refs" -d 'List recent extract/catalog inventory under ~/.aicx/'
complete -c aicx -n "__fish_aicx_needs_command" -f -a "state" -d 'Manage extraction dedup state (watermarks and hashes)'
complete -c aicx -n "__fish_aicx_needs_command" -f -a "dashboard" -d 'Generate a searchable HTML dashboard from extracts/catalog, or serve it locally'
...
complete -c aicx -n "__fish_aicx_needs_command" -f -a "read" -d 'Read one legacy archive chunk by path, file name, or `chunk:<id>` reference'
complete -c aicx -n "__fish_aicx_needs_command" -f -a "open" -d 'Read one legacy archive chunk by path, file name, or `chunk:<id>` reference'
complete -c aicx -n "__fish_aicx_needs_command" -f -a "steer" -d 'Retrieve chunks by metadata from the published CURRENT index'
complete -c aicx -n "__fish_aicx_needs_command" -f -a "migrate" -d 'Migrate legacy ~/.ai-contexters/ artifacts into the AICX legacy archive'
complete -c aicx -n "__fish_aicx_needs_command" -f -a "migrate-intent-schema" -d 'Classify legacy archive chunks into 11-type intent entries and report counts'
complete -c aicx -n "__fish_aicx_needs_command" -f -a "doctor" -d 'Diagnose and optionally repair AICX home health (index, state, legacy archive)'
...
complete -c aicx -n "__fish_aicx_using_subcommand overlay" -l rebuild -d 'Re-evaluate every typed claim while preserving persisted intent ids'
...
complete -c aicx -n "__fish_aicx_using_subcommand claude" -s o -l output -d 'Output directory for explicit report files' -r -F
...
complete -c aicx -n "__fish_aicx_using_subcommand claude" -l emit -d 'What to print to stdout: paths, json, none (default: none)' -r -f -a "paths\t'Print legacy archive chunk paths (one per line)'
json\t'Print JSON report (includes the compatibility `store_paths` field)'
...
complete -c aicx -n "__fish_aicx_using_subcommand codex" -s o -l output -d 'Output directory for explicit report files' -r -F
...
complete -c aicx -n "__fish_aicx_using_subcommand codex" -l emit -d 'What to print to stdout: paths, json, none (default: none)' -r -f -a "paths\t'Print legacy archive chunk paths (one per line)'
json\t'Print JSON report (includes the compatibility `store_paths` field)'
none\t'Print nothing to stdout'"
...
complete -c aicx -n "__fish_aicx_using_subcommand all" -s o -l output -d 'Output directory for explicit report files' -r -F
...
complete -c aicx -n "__fish_aicx_using_subcommand all" -l emit -d 'What to print to stdout: paths, json, none (default: none)' -r -f -a "paths\t'Print legacy archive chunk paths (one per line)'
json\t'Print JSON report (includes the compatibility `store_paths` field)'
none\t'Print nothing to stdout'"
...
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from codex" -s H -l hours -d 'Hours to look back inside the session view (0 = unbounded). A window on the projection, not a re-parse; absence is never a silent default' -r
...
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from codex" -l lineage -d 'Walk parent sessions through `session_meta.forked_from_id` (session catalog lookup, never filename guessing). Bare `--lineage` = unbounded; `--lineage=N` = at most N parents. Needs --session (catalog)' -r
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from codex" -l result -d 'Retained shell result body: none (`$ cmd [N lines, sha256:…]`, default) | head=N | full. The body stays in the substrate either way' -r
...
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from codex" -l agent-only -d 'Only include assistant answers (`assistant_final`). Reasoning, inter-agent traffic and lineage metadata are excluded: they are not the assistant speaking to the operator'
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from codex" -l user-commands -d 'Only shell commands the human submitted (e.g. Codex `<user_shell_command>`). A command quoted in prose is not an execution'
...
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from claude" -s p -l project -d 'Project/repo filter, repeatable or comma-separated (OR across projects, AND with every other axis). Entries are kept when their recorded cwd belongs to one of them; an entry with no known cwd is filtered out rather than guessed. A single value also names the output\'s project identity, as before' -r
...
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from claude" -s H -l hours -d 'Hours to look back inside the session view (0 = unbounded). A window on the projection, not a re-parse; absence is never a silent default' -r
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from claude" -l kind -d 'Throne kind filter (repeatable / comma list): human | echo_seal | shell_action | inject | assistant_final | lineage_meta | inter_agent. `inter_agent` opens the inter-agent lane; it is never rendered as assistant and stays outside --conversation / --dialog by default' -r
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from claude" -l lineage -d 'Walk parent sessions through `session_meta.forked_from_id` (session catalog lookup, never filename guessing). Bare `--lineage` = unbounded; `--lineage=N` = at most N parents. Needs --session (catalog)' -r
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from claude" -l result -d 'Retained shell result body: none (`$ cmd [N lines, sha256:…]`, default) | head=N | full. The body stays in the substrate either way' -r
...
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from claude" -l agent-only -d 'Only include assistant answers (`assistant_final`). Reasoning, inter-agent traffic and lineage metadata are excluded: they are not the assistant speaking to the operator'
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from claude" -l user-commands -d 'Only shell commands the human submitted (e.g. Codex `<user_shell_command>`). A command quoted in prose is not an execution'
...
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from gemini" -s p -l project -d 'Project/repo filter, repeatable or comma-separated (OR across projects, AND with every other axis). Entries are kept when their recorded cwd belongs to one of them; an entry with no known cwd is filtered out rather than guessed. A single value also names the output\'s project identity, as before' -r
...
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from gemini" -s H -l hours -d 'Hours to look back inside the session view (0 = unbounded). A window on the projection, not a re-parse; absence is never a silent default' -r
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from gemini" -l kind -d 'Throne kind filter (repeatable / comma list): human | echo_seal | shell_action | inject | assistant_final | lineage_meta | inter_agent. `inter_agent` opens the inter-agent lane; it is never rendered as assistant and stays outside --conversation / --dialog by default' -r
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from gemini" -l lineage -d 'Walk parent sessions through `session_meta.forked_from_id` (session catalog lookup, never filename guessing). Bare `--lineage` = unbounded; `--lineage=N` = at most N parents. Needs --session (catalog)' -r
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from gemini" -l result -d 'Retained shell result body: none (`$ cmd [N lines, sha256:…]`, default) | head=N | full. The body stays in the substrate either way' -r
...
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from gemini" -l agent-only -d 'Only include assistant answers (`assistant_final`). Reasoning, inter-agent traffic and lineage metadata are excluded: they are not the assistant speaking to the operator'
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from gemini" -l user-commands -d 'Only shell commands the human submitted (e.g. Codex `<user_shell_command>`). A command quoted in prose is not an execution'
...
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from grok" -s p -l project -d 'Project/repo filter, repeatable or comma-separated (OR across projects, AND with every other axis). Entries are kept when their recorded cwd belongs to one of them; an entry with no known cwd is filtered out rather than guessed. A single value also names the output\'s project identity, as before' -r
...
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from grok" -s H -l hours -d 'Hours to look back inside the session view (0 = unbounded). A window on the projection, not a re-parse; absence is never a silent default' -r
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from grok" -l kind -d 'Throne kind filter (repeatable / comma list): human | echo_seal | shell_action | inject | assistant_final | lineage_meta | inter_agent. `inter_agent` opens the inter-agent lane; it is never rendered as assistant and stays outside --conversation / --dialog by default' -r
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from grok" -l lineage -d 'Walk parent sessions through `session_meta.forked_from_id` (session catalog lookup, never filename guessing). Bare `--lineage` = unbounded; `--lineage=N` = at most N parents. Needs --session (catalog)' -r
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from grok" -l result -d 'Retained shell result body: none (`$ cmd [N lines, sha256:…]`, default) | head=N | full. The body stays in the substrate either way' -r
...
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from grok" -l agent-only -d 'Only include assistant answers (`assistant_final`). Reasoning, inter-agent traffic and lineage metadata are excluded: they are not the assistant speaking to the operator'
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from grok" -l user-commands -d 'Only shell commands the human submitted (e.g. Codex `<user_shell_command>`). A command quoted in prose is not an execution'
...
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from junie" -s p -l project -d 'Project/repo filter, repeatable or comma-separated (OR across projects, AND with every other axis). Entries are kept when their recorded cwd belongs to one of them; an entry with no known cwd is filtered out rather than guessed. A single value also names the output\'s project identity, as before' -r
...
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from junie" -s H -l hours -d 'Hours to look back inside the session view (0 = unbounded). A window on the projection, not a re-parse; absence is never a silent default' -r
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from junie" -l kind -d 'Throne kind filter (repeatable / comma list): human | echo_seal | shell_action | inject | assistant_final | lineage_meta | inter_agent. `inter_agent` opens the inter-agent lane; it is never rendered as assistant and stays outside --conversation / --dialog by default' -r
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from junie" -l lineage -d 'Walk parent sessions through `session_meta.forked_from_id` (session catalog lookup, never filename guessing). Bare `--lineage` = unbounded; `--lineage=N` = at most N parents. Needs --session (catalog)' -r
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from junie" -l result -d 'Retained shell result body: none (`$ cmd [N lines, sha256:…]`, default) | head=N | full. The body stays in the substrate either way' -r
...
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from junie" -l agent-only -d 'Only include assistant answers (`assistant_final`). Reasoning, inter-agent traffic and lineage metadata are excluded: they are not the assistant speaking to the operator'
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from junie" -l user-commands -d 'Only shell commands the human submitted (e.g. Codex `<user_shell_command>`). A command quoted in prose is not an execution'
...
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from kimi" -l session -d 'Session id: source id, logical id, alias, UUID suffix (≥8 chars), or unique prefix. Resolved through the session catalog before any parse' -r
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from kimi" -l file -d 'Direct source file. Builds a source handle from this path only — no catalog scan, no global AICX state. Requires `-o/--output`' -r -F
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from kimi" -s o -l output -d 'Output file path. Required with `--file`; defaults to `~/.aicx/extracts/<agent>/<session_id>[_conversation][_user].md` in session mode' -r -F
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from kimi" -s p -l project -d 'Project/repo filter, repeatable or comma-separated (OR across projects, AND with every other axis). Entries are kept when their recorded cwd belongs to one of them; an entry with no known cwd is filtered out rather than guessed. A single value also names the output\'s project identity, as before' -r
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from kimi" -l max-message-chars -d 'Maximum message characters in markdown (0 = no truncation)' -r
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from kimi" -s H -l hours -d 'Hours to look back inside the session view (0 = unbounded). A window on the projection, not a re-parse; absence is never a silent default' -r
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from kimi" -l kind -d 'Throne kind filter (repeatable / comma list): human | echo_seal | shell_action | inject | assistant_final | lineage_meta | inter_agent. `inter_agent` opens the inter-agent lane; it is never rendered as assistant and stays outside --conversation / --dialog by default' -r
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from kimi" -l lineage -d 'Walk parent sessions through `session_meta.forked_from_id` (session catalog lookup, never filename guessing). Bare `--lineage` = unbounded; `--lineage=N` = at most N parents. Needs --session (catalog)' -r
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from kimi" -l result -d 'Retained shell result body: none (`$ cmd [N lines, sha256:…]`, default) | head=N | full. The body stays in the substrate either way' -r
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from kimi" -l no-redact-secrets -d 'Redact secrets (tokens/keys) from outputs before writing/syncing'
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from kimi" -l user-only -d 'Only include user messages (exclude assistant + reasoning)'
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from kimi" -l agent-only -d 'Only include assistant answers (`assistant_final`). Reasoning, inter-agent traffic and lineage metadata are excluded: they are not the assistant speaking to the operator'
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from kimi" -l user-commands -d 'Only shell commands the human submitted (e.g. Codex `<user_shell_command>`). A command quoted in prose is not an execution'
...
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from all" -l kind -d 'Throne kind filter (repeatable / comma list), same vocabulary as `aicx extract <agent> --kind`' -r
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from all" -l result -d 'Retained shell result body: none (default) | head=N | full' -r
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from all" -l max-message-chars -d 'Maximum message characters (0 = no truncation)' -r
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from all" -s o -l output -d 'Output root for the extracts. Defaults to `<AICX_HOME>/extracts`' -r -F
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from all" -l no-redact-secrets -d 'Redact secrets (tokens/keys) from outputs before writing/syncing'
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from all" -l user-only -d 'Only user messages'
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from all" -l agent-only -d 'Only assistant answers (`assistant_final`)'
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from all" -l user-commands -d 'Only shell commands the human submitted'
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from all" -l agent-commands -d 'Only tool / shell invocations the agent made'
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from all" -l conversation -d 'Conversation-first rendering'
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from all" -l rebuild -l force -d 'Re-materialize every selected session, ignoring the incremental state'
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from all" -l dry-run -d 'Plan only: report what each source would become, write nothing'
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from all" -l json -d 'Emit the run manifest as JSON on stdout instead of the human summary'
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from all" -s v -l verbose -d 'Verbose diagnostics: echo per-file extractor warnings to stderr'
complete -c aicx -n "__fish_aicx_using_subcommand extract; and __fish_seen_subcommand_from all" -l project-fuzzy -d 'Opt in to project-family matching. By default project filters are exact and an ambiguous bare repository name fails closed'
...
complete -c aicx -n "__fish_aicx_using_subcommand catalog; and not __fish_seen_subcommand_from rebuild refresh status resolve help" -s v -l verbose -d 'Verbose diagnostics: echo per-file extractor warnings to stderr'
complete -c aicx -n "__fish_aicx_using_subcommand catalog; and not __fish_seen_subcommand_from rebuild refresh status resolve help" -l project-fuzzy -d 'Opt in to project-family matching. By default project filters are exact and an ambiguous bare repository name fails closed'
complete -c aicx -n "__fish_aicx_using_subcommand catalog; and not __fish_seen_subcommand_from rebuild refresh status resolve help" -s h -l help -d 'Print help (see more with \'--help\')'
complete -c aicx -n "__fish_aicx_using_subcommand catalog; and not __fish_seen_subcommand_from rebuild refresh status resolve help" -f -a "rebuild" -d 'Walk all source roots and rewrite `~/.aicx/catalog/sessions.jsonl`'
complete -c aicx -n "__fish_aicx_using_subcommand catalog; and not __fish_seen_subcommand_from rebuild refresh status resolve help" -f -a "refresh" -d 'Admit new or changed sessions from a bounded hot window'
complete -c aicx -n "__fish_aicx_using_subcommand catalog; and not __fish_seen_subcommand_from rebuild refresh status resolve help" -f -a "status" -d 'Compare durable catalog fingerprints to live agent sources (no write)'
...
complete -c aicx -n "__fish_aicx_using_subcommand catalog; and __fish_seen_subcommand_from refresh" -s H -l hours -d 'Window in hours' -r
complete -c aicx -n "__fish_aicx_using_subcommand catalog; and __fish_seen_subcommand_from refresh" -l json -d 'Emit JSON report to stdout'
complete -c aicx -n "__fish_aicx_using_subcommand catalog; and __fish_seen_subcommand_from refresh" -s v -l verbose -d 'Verbose diagnostics: echo per-file extractor warnings to stderr'
complete -c aicx -n "__fish_aicx_using_subcommand catalog; and __fish_seen_subcommand_from refresh" -l project-fuzzy -d 'Opt in to project-family matching. By default project filters are exact and an ambiguous bare repository name fails closed'
complete -c aicx -n "__fish_aicx_using_subcommand catalog; and __fish_seen_subcommand_from refresh" -s h -l help -d 'Print help (see more with \'--help\')'
complete -c aicx -n "__fish_aicx_using_subcommand catalog; and __fish_seen_subcommand_from status" -l json -d 'Emit JSON (`aicx.catalog.status.v1`)'
complete -c aicx -n "__fish_aicx_using_subcommand catalog; and __fish_seen_subcommand_from status" -s v -l verbose -d 'Verbose diagnostics: echo per-file extractor warnings to stderr'
complete -c aicx -n "__fish_aicx_using_subcommand catalog; and __fish_seen_subcommand_from status" -l project-fuzzy -d 'Opt in to project-family matching. By default project filters are exact and an ambiguous bare repository name fails closed'
complete -c aicx -n "__fish_aicx_using_subcommand catalog; and __fish_seen_subcommand_from status" -s h -l help -d 'Print help (see more with \'--help\')'
...
complete -c aicx -n "__fish_aicx_using_subcommand catalog; and __fish_seen_subcommand_from help" -f -a "refresh" -d 'Admit new or changed sessions from a bounded hot window'
complete -c aicx -n "__fish_aicx_using_subcommand catalog; and __fish_seen_subcommand_from help" -f -a "status" -d 'Compare durable catalog fingerprints to live agent sources (no write)'
...
complete -c aicx -n "__fish_aicx_using_subcommand store; and not __fish_seen_subcommand_from rebuild refresh status resolve help" -s v -l verbose -d 'Verbose diagnostics: echo per-file extractor warnings to stderr'
complete -c aicx -n "__fish_aicx_using_subcommand store; and not __fish_seen_subcommand_from rebuild refresh status resolve help" -l project-fuzzy -d 'Opt in to project-family matching. By default project filters are exact and an ambiguous bare repository name fails closed'
complete -c aicx -n "__fish_aicx_using_subcommand store; and not __fish_seen_subcommand_from rebuild refresh status resolve help" -s h -l help -d 'Print help (see more with \'--help\')'
complete -c aicx -n "__fish_aicx_using_subcommand store; and not __fish_seen_subcommand_from rebuild refresh status resolve help" -f -a "rebuild" -d 'Walk all source roots and rewrite `~/.aicx/catalog/sessions.jsonl`'
complete -c aicx -n "__fish_aicx_using_subcommand store; and not __fish_seen_subcommand_from rebuild refresh status resolve help" -f -a "refresh" -d 'Admit new or changed sessions from a bounded hot window'
complete -c aicx -n "__fish_aicx_using_subcommand store; and not __fish_seen_subcommand_from rebuild refresh status resolve help" -f -a "status" -d 'Compare durable catalog fingerprints to live agent sources (no write)'
...
complete -c aicx -n "__fish_aicx_using_subcommand store; and __fish_seen_subcommand_from refresh" -s H -l hours -d 'Window in hours' -r
complete -c aicx -n "__fish_aicx_using_subcommand store; and __fish_seen_subcommand_from refresh" -l json -d 'Emit JSON report to stdout'
complete -c aicx -n "__fish_aicx_using_subcommand store; and __fish_seen_subcommand_from refresh" -s v -l verbose -d 'Verbose diagnostics: echo per-file extractor warnings to stderr'
complete -c aicx -n "__fish_aicx_using_subcommand store; and __fish_seen_subcommand_from refresh" -l project-fuzzy -d 'Opt in to project-family matching. By default project filters are exact and an ambiguous bare repository name fails closed'
complete -c aicx -n "__fish_aicx_using_subcommand store; and __fish_seen_subcommand_from refresh" -s h -l help -d 'Print help (see more with \'--help\')'
complete -c aicx -n "__fish_aicx_using_subcommand store; and __fish_seen_subcommand_from status" -l json -d 'Emit JSON (`aicx.catalog.status.v1`)'
complete -c aicx -n "__fish_aicx_using_subcommand store; and __fish_seen_subcommand_from status" -s v -l verbose -d 'Verbose diagnostics: echo per-file extractor warnings to stderr'
complete -c aicx -n "__fish_aicx_using_subcommand store; and __fish_seen_subcommand_from status" -l project-fuzzy -d 'Opt in to project-family matching. By default project filters are exact and an ambiguous bare repository name fails closed'
complete -c aicx -n "__fish_aicx_using_subcommand store; and __fish_seen_subcommand_from status" -s h -l help -d 'Print help (see more with \'--help\')'
complete -c aicx -n "__fish_aicx_using_subcommand store; and __fish_seen_subcommand_from resolve" -l json -d 'Emit JSON'
complete -c aicx -n "__fish_aicx_using_subcommand store; and __fish_seen_subcommand_from resolve" -s v -l verbose -d 'Verbose diagnostics: echo per-file extractor warnings to stderr'
complete -c aicx -n "__fish_aicx_using_subcommand store; and __fish_seen_subcommand_from resolve" -l project-fuzzy -d 'Opt in to project-family matching. By default project filters are exact and an ambiguous bare repository name fails closed'
complete -c aicx -n "__fish_aicx_using_subcommand store; and __fish_seen_subcommand_from resolve" -s h -l help -d 'Print help (see more with \'--help\')'
complete -c aicx -n "__fish_aicx_using_subcommand store; and __fish_seen_subcommand_from help" -f -a "rebuild" -d 'Walk all source roots and rewrite `~/.aicx/catalog/sessions.jsonl`'
complete -c aicx -n "__fish_aicx_using_subcommand store; and __fish_seen_subcommand_from help" -f -a "refresh" -d 'Admit new or changed sessions from a bounded hot window'
complete -c aicx -n "__fish_aicx_using_subcommand store; and __fish_seen_subcommand_from help" -f -a "status" -d 'Compare durable catalog fingerprints to live agent sources (no write)'
complete -c aicx -n "__fish_aicx_using_subcommand store; and __fish_seen_subcommand_from help" -f -a "resolve" -d 'Resolve one session id from the durable catalog'
complete -c aicx -n "__fish_aicx_using_subcommand store; and __fish_seen_subcommand_from help" -f -a "help" -d 'Print this message or the help of the given subcommand(s)'
...
complete -c aicx -n "__fish_aicx_using_subcommand ingest" -l emit -d 'What to print to stdout: paths, json, none (default: none)' -r -f -a "paths\t'Print legacy archive chunk paths (one per line)'
json\t'Print JSON report (includes the compatibility `store_paths` field)'
none\t'Print nothing to stdout'"
...
complete -c aicx -n "__fish_aicx_using_subcommand sessions; and __fish_seen_subcommand_from list" -s j -l json -d 'Alias for `--format json` (vc-frame / plugin assimilation)'
...
complete -c aicx -n "__fish_aicx_using_subcommand sessions; and __fish_seen_subcommand_from show" -s j -l json -d 'Alias for `--format json` (vc-frame / plugin assimilation)'
...
complete -c aicx -n "__fish_aicx_using_subcommand wizard" -l view -d 'Open a specific screen: corpus | doctor | intents | rebuild | search' -r
complete -c aicx -n "__fish_aicx_using_subcommand wizard" -l query -d 'Initial search query when `--view search` (or global prefill for Search)' -r
complete -c aicx -n "__fish_aicx_using_subcommand wizard" -s p -l project -d 'Optional project filter for Search/Intents entry (exact, fail-closed)' -r
complete -c aicx -n "__fish_aicx_using_subcommand wizard" -s a -l agent -d 'Optional agent filter for Search/Intents entry' -r
...
complete -c aicx -n "__fish_aicx_using_subcommand dashboard" -l aicx-home -d 'AICX home directory (default: ~/.aicx)' -r -F
...
complete -c aicx -n "__fish_aicx_using_subcommand dashboard-serve" -l aicx-home -d 'AICX home directory (default: ~/.aicx)' -r -F
...
complete -c aicx -n "__fish_aicx_using_subcommand intents" -s p -l project -d 'Catalog project filters. Omit to scan all projects. Repeated `-p` flags or comma list (`-p a,b`) form a union' -r
...
complete -c aicx -n "__fish_aicx_using_subcommand intents" -l live -d 'Force the live source-root scan regardless of the window size. (Windows of ≤ 48h scan live sources automatically.)'
complete -c aicx -n "__fish_aicx_using_subcommand intents" -l no-live -d 'Disable the automatic live scan for hot (≤ 48h) windows'
...
complete -c aicx -n "__fish_aicx_using_subcommand continuity; and not __fish_seen_subcommand_from show write help" -s v -l verbose -d 'Verbose diagnostics: echo per-file extractor warnings to stderr'
complete -c aicx -n "__fish_aicx_using_subcommand continuity; and not __fish_seen_subcommand_from show write help" -l project-fuzzy -d 'Opt in to project-family matching. By default project filters are exact and an ambiguous bare repository name fails closed'
complete -c aicx -n "__fish_aicx_using_subcommand continuity; and not __fish_seen_subcommand_from show write help" -s h -l help -d 'Print help (see more with \'--help\')'
complete -c aicx -n "__fish_aicx_using_subcommand continuity; and not __fish_seen_subcommand_from show write help" -f -a "show" -d 'Render the continuity pack to stdout'
complete -c aicx -n "__fish_aicx_using_subcommand continuity; and not __fish_seen_subcommand_from show write help" -f -a "write" -d 'Write the continuity pack to a file'
...
complete -c aicx -n "__fish_aicx_using_subcommand continuity; and __fish_seen_subcommand_from show" -l project-fuzzy -d 'Opt in to project-family matching. By default project filters are exact and an ambiguous bare repository name fails closed'
...
complete -c aicx -n "__fish_aicx_using_subcommand continuity; and __fish_seen_subcommand_from write" -l project-fuzzy -d 'Opt in to project-family matching. By default project filters are exact and an ambiguous bare repository name fails closed'
complete -c aicx -n "__fish_aicx_using_subcommand continuity; and __fish_seen_subcommand_from write" -s h -l help -d 'Print help (see more with \'--help\')'
complete -c aicx -n "__fish_aicx_using_subcommand continuity; and __fish_seen_subcommand_from help" -f -a "show" -d 'Render the continuity pack to stdout'
complete -c aicx -n "__fish_aicx_using_subcommand continuity; and __fish_seen_subcommand_from help" -f -a "write" -d 'Write the continuity pack to a file'
complete -c aicx -n "__fish_aicx_using_subcommand continuity; and __fish_seen_subcommand_from help" -f -a "help" -d 'Print this message or the help of the given subcommand(s)'
complete -c aicx -n "__fish_aicx_using_subcommand tail" -s p -l project -d 'Catalog project filters. Omit to scan all projects. Repeated `-p` flags or comma list (`-p a,b`) form a union' -r
...
false\t''"
complete -c aicx -n "__fish_aicx_using_subcommand serve" -l refresh-interval-seconds -d 'Accepted for compatibility; ignored unless `--experimental-auto-refresh` is set' -r
...
complete -c aicx -n "__fish_aicx_using_subcommand serve" -l experimental-auto-refresh -d 'Explicitly opt in to the legacy embedded HTTP catalog/index writer'
complete -c aicx -n "__fish_aicx_using_subcommand serve" -l no-auto-refresh -d 'Deprecated compatibility flag: auto-refresh is already disabled by default'
...
complete -c aicx -n "__fish_aicx_using_subcommand search" -l lineage -d 'Accepted for grammar parity with `extract`; hits are chunks, so the parent walk is reported as not applied here' -r
...
complete -c aicx -n "__fish_aicx_using_subcommand search" -l dialog -d 'Dialogue view of hits: delayed human speech (echo-bus / queue) rendered as speech, with its seals'
...
complete -c aicx -n "__fish_aicx_using_subcommand index; and not __fish_seen_subcommand_from status derive help" -l dry-run -d 'Preview only. Omit this flag to materialize the persistent lexical CURRENT used by `aicx search`. Incompatible with `--semantic`' -r -f -a "true\t''
false\t''"
...
complete -c aicx -n "__fish_aicx_using_subcommand index; and not __fish_seen_subcommand_from status derive help" -l cache-extracts -d 'Compatibility no-op: extracts and the reuse ledger are always written now, so a second `aicx index` re-parses only changed sessions. Accepted so wizard and scripts keep working'
complete -c aicx -n "__fish_aicx_using_subcommand index; and not __fish_seen_subcommand_from status derive help" -l semantic -d 'Opt-in dense semantic generation: embed session extracts and publish `dense.exact_mmap_v1.bin` beside Tantivy in CURRENT. Prefer one workstation owner; laptops stay on plain `aicx index`'
...
complete -c aicx -n "__fish_aicx_using_subcommand steer" -s p -l project -d 'Catalog project filters. Omit to search all projects. Repeated `-p` flags or comma list (`-p a,b`) form a union' -r
...
complete -c aicx -n "__fish_aicx_using_subcommand migrate" -l legacy-root -d 'Override legacy input archive root (default: ~/.ai-contexters)' -r -F
complete -c aicx -n "__fish_aicx_using_subcommand migrate" -l aicx-home -d 'Override AICX home (default: ~/.aicx)' -r -F
complete -c aicx -n "__fish_aicx_using_subcommand migrate" -l cards-v2 -d 'Upgrade legacy archive cards v1 -> v2 in place (sidecar schema/honesty fields, bracket header -> YAML frontmatter; body bytes never change). Optional ROOT overrides the walked directory (default: legacy archive dir). Dry-run by default; pass --apply to write' -r -F
...
complete -c aicx -n "__fish_aicx_using_subcommand migrate" -l no-intent-schema -d 'Skip post-migration intent schema scan on the legacy archive'
...
complete -c aicx -n "__fish_aicx_using_subcommand migrate-intent-schema" -s p -l project -d 'Strict project filter: `owner/repo`, `/repo` (cross-org repo name), `owner/` (org wildcard), or a unique exact `name`. Omit to scan the whole legacy archive. Substring matching is intentionally disabled' -r
complete -c aicx -n "__fish_aicx_using_subcommand migrate-intent-schema" -l aicx-home -d 'Override AICX home (default: ~/.aicx)' -r -F
...
complete -c aicx -n "__fish_aicx_using_subcommand doctor" -l clean-retired-steer -d 'Remove obsolete steer-only artifacts. This never mutates or rebuilds CURRENT; use `aicx index` for the canonical retrieval generation'
...
complete -c aicx -n "__fish_aicx_using_subcommand doctor" -l dry-run -d 'With --fix-buckets, preview the planned canonicalize/quarantine actions without modifying the filesystem. Output entries are prefixed with `[dry-run]`. Use this before running `--fix-buckets` against a large legacy archive to verify the classification before commit'
...
complete -c aicx -n "__fish_aicx_using_subcommand doctor" -l migrate-identities -d 'Plan the one-time project-identity migration: persisted index.json key casing → GitHub nameWithOwner canon (lowercase, the fresh-derivation form), legacy `store/` directory normalization, historical-card alias map (annotate-only, cards never rewritten), and a typo-twin bucket report. Dry-run by default — writes migration/identity-manifest.json + identity-report.md with zero archive mutation; add --apply to execute the planned renames'
complete -c aicx -n "__fish_aicx_using_subcommand doctor" -l apply -d 'With --prune-empty-bodies, move empty-body chunks into recoverable quarantine. With --migrate-identities, execute the planned identity renames. Refuses to combine with --dry-run: on a archive-mutating surface the preview flag must always win, so the ambiguous combination is a parse error'
complete -c aicx -n "__fish_aicx_using_subcommand doctor" -s y -l yes -d 'Assume yes on doctor cleanup prompts'
complete -c aicx -n "__fish_aicx_using_subcommand doctor" -l repair-runtime -d 'Repair the macOS background runtime in one pass: preserve the existing MCP bind configuration, point launchd at the public AICX launcher, disable in-process refresh, reload the service, and install the short-lived catalog/index scheduler (every 8640s)'
...
complete -c aicx -n "__fish_aicx_using_subcommand doctor" -l check-dedup -d 'Report duplicate content_sha256 groups across legacy archive and context-corpus'
...
complete -c aicx -n "__fish_aicx_using_subcommand help; and not __fish_seen_subcommand_from completions overlay claude codex all extract conversations catalog ingest list sources sessions claims results clarify wizard refs state dashboard reports corpus reports-extractor dashboard-serve intents continuity tail serve init search eval index config read steer migrate migrate-intent-schema doctor health warmup help" -f -a "overlay" -d 'Join catalog typed intents to the current Loctree anchor catalog'
...
complete -c aicx -n "__fish_aicx_using_subcommand help; and not __fish_seen_subcommand_from completions overlay claude codex all extract conversations catalog ingest list sources sessions claims results clarify wizard refs state dashboard reports corpus reports-extractor dashboard-serve intents continuity tail serve init search eval index config read steer migrate migrate-intent-schema doctor health warmup help" -f -a "extract" -d 'Extract sessions — one at a time, or every compatible source at once'
complete -c aicx -n "__fish_aicx_using_subcommand help; and not __fish_seen_subcommand_from completions overlay claude codex all extract conversations catalog ingest list sources sessions claims results clarify wizard refs state dashboard reports corpus reports-extractor dashboard-serve intents continuity tail serve init search eval index config read steer migrate migrate-intent-schema doctor health warmup help" -f -a "conversations" -d 'Batch-export conversation JSON files without writing per-frame card files'
...
complete -c aicx -n "__fish_aicx_using_subcommand help; and not __fish_seen_subcommand_from completions overlay claude codex all extract conversations catalog ingest list sources sessions claims results clarify wizard refs state dashboard reports corpus reports-extractor dashboard-serve intents continuity tail serve init search eval index config read steer migrate migrate-intent-schema doctor health warmup help" -f -a "wizard" -d 'Interactive daily-driver entrypoint for catalog, extracts, index, and doctor'
complete -c aicx -n "__fish_aicx_using_subcommand help; and not __fish_seen_subcommand_from completions overlay claude codex all extract conversations catalog ingest list sources sessions claims results clarify wizard refs state dashboard reports corpus reports-extractor dashboard-serve intents continuity tail serve init search eval index config read steer migrate migrate-intent-schema doctor health warmup help" -f -a "refs" -d 'List recent extract/catalog inventory under ~/.aicx/'
complete -c aicx -n "__fish_aicx_using_subcommand help; and not __fish_seen_subcommand_from completions overlay claude codex all extract conversations catalog ingest list sources sessions claims results clarify wizard refs state dashboard reports corpus reports-extractor dashboard-serve intents continuity tail serve init search eval index config read steer migrate migrate-intent-schema doctor health warmup help" -f -a "state" -d 'Manage extraction dedup state (watermarks and hashes)'
complete -c aicx -n "__fish_aicx_using_subcommand help; and not __fish_seen_subcommand_from completions overlay claude codex all extract conversations catalog ingest list sources sessions claims results clarify wizard refs state dashboard reports corpus reports-extractor dashboard-serve intents continuity tail serve init search eval index config read steer migrate migrate-intent-schema doctor health warmup help" -f -a "dashboard" -d 'Generate a searchable HTML dashboard from extracts/catalog, or serve it locally'
...
complete -c aicx -n "__fish_aicx_using_subcommand help; and not __fish_seen_subcommand_from completions overlay claude codex all extract conversations catalog ingest list sources sessions claims results clarify wizard refs state dashboard reports corpus reports-extractor dashboard-serve intents continuity tail serve init search eval index config read steer migrate migrate-intent-schema doctor health warmup help" -f -a "read" -d 'Read one legacy archive chunk by path, file name, or `chunk:<id>` reference'
complete -c aicx -n "__fish_aicx_using_subcommand help; and not __fish_seen_subcommand_from completions overlay claude codex all extract conversations catalog ingest list sources sessions claims results clarify wizard refs state dashboard reports corpus reports-extractor dashboard-serve intents continuity tail serve init search eval index config read steer migrate migrate-intent-schema doctor health warmup help" -f -a "steer" -d 'Retrieve chunks by metadata from the published CURRENT index'
complete -c aicx -n "__fish_aicx_using_subcommand help; and not __fish_seen_subcommand_from completions overlay claude codex all extract conversations catalog ingest list sources sessions claims results clarify wizard refs state dashboard reports corpus reports-extractor dashboard-serve intents continuity tail serve init search eval index config read steer migrate migrate-intent-schema doctor health warmup help" -f -a "migrate" -d 'Migrate legacy ~/.ai-contexters/ artifacts into the AICX legacy archive'
complete -c aicx -n "__fish_aicx_using_subcommand help; and not __fish_seen_subcommand_from completions overlay claude codex all extract conversations catalog ingest list sources sessions claims results clarify wizard refs state dashboard reports corpus reports-extractor dashboard-serve intents continuity tail serve init search eval index config read steer migrate migrate-intent-schema doctor health warmup help" -f -a "migrate-intent-schema" -d 'Classify legacy archive chunks into 11-type intent entries and report counts'
complete -c aicx -n "__fish_aicx_using_subcommand help; and not __fish_seen_subcommand_from completions overlay claude codex all extract conversations catalog ingest list sources sessions claims results clarify wizard refs state dashboard reports corpus reports-extractor dashboard-serve intents continuity tail serve init search eval index config read steer migrate migrate-intent-schema doctor health warmup help" -f -a "doctor" -d 'Diagnose and optionally repair AICX home health (index, state, legacy archive)'
...
complete -c aicx -n "__fish_aicx_using_subcommand help; and __fish_seen_subcommand_from catalog" -f -a "status" -d 'Compare durable catalog fingerprints to live agent sources (no write)'
...
complete -c aicx -n "__fish_aicx_using_subcommand help; and __fish_seen_subcommand_from continuity" -f -a "show" -d 'Render the continuity pack to stdout'
complete -c aicx -n "__fish_aicx_using_subcommand help; and __fish_seen_subcommand_from continuity" -f -a "write" -d 'Write the continuity pack to a file'
