...
complete -c aicx -n "__fish_aicx_needs_command" -f -a "claude" -d 'Extract and store Agents\' sessions into the canonical corpus (canonical corpus extraction)'
complete -c aicx -n "__fish_aicx_needs_command" -f -a "codex" -d 'Extract and store Codex sessions into the canonical corpus'
complete -c aicx -n "__fish_aicx_needs_command" -f -a "all" -d 'Extract and store from all agents (Claude + Codex + Gemini + Junie + Codescribe) into the canonical corpus'
...
complete -c aicx -n "__fish_aicx_needs_command" -f -a "store" -d 'Build the canonical corpus in from local agents\' session files'
...
complete -c aicx -n "__fish_aicx_needs_command" -f -a "search" -d 'Search the canonical corpus. Semantic by default; automatic filesystem-fuzzy fallback when semantic search is unavailable'
complete -c aicx -n "__fish_aicx_needs_command" -f -a "eval" -d 'Run local evaluation helpers for retrieval/search quality'
complete -c aicx -n "__fish_aicx_needs_command" -f -a "index" -d 'Catch up the canonical corpus, then build the semantic index. Use `--dry-run` to preview without writing'
...
complete -c aicx -n "__fish_aicx_needs_command" -f -a "health" -d 'Emit the full AICX health report as JSON for automation'
...
complete -c aicx -n "__fish_aicx_using_subcommand store" -s p -l project -d 'Source cwd/project filter(s): narrows session discovery before repo segmentation' -r
complete -c aicx -n "__fish_aicx_using_subcommand store" -s a -l agent -d 'Agent filter: one of claude, codex, gemini, junie, grok, codescribe, operator-md. Default: claude+codex+gemini+junie+grok+codescribe (operator-md is opt-in via `--agent operator-md`)' -r -f -a "claude\t''
codex\t''
gemini\t''
junie\t''
grok\t''
codescribe\t''
operator-md\t''"
complete -c aicx -n "__fish_aicx_using_subcommand store" -s H -l hours -d 'Hours to look back (default: 48, 0 = all time)' -r
complete -c aicx -n "__fish_aicx_using_subcommand store" -l emit -d 'What to print to stdout: paths, json, none (default: none)' -r -f -a "paths\t'Print store chunk paths (one per line)'
json\t'Print JSON report (includes `store_paths` for convenience)'
none\t'Print nothing to stdout'"
complete -c aicx -n "__fish_aicx_using_subcommand store" -l no-redact-secrets -d 'Redact secrets (tokens/keys) from outputs before writing/syncing'
complete -c aicx -n "__fish_aicx_using_subcommand store" -l full-rescan -d 'Ignore the stored watermark and previously-seen hashes for this run'
complete -c aicx -n "__fish_aicx_using_subcommand store" -l incremental -d 'Legacy no-op: incremental mode is now the default'
complete -c aicx -n "__fish_aicx_using_subcommand store" -l user-only -d 'Only include user messages (exclude assistant + reasoning)'
complete -c aicx -n "__fish_aicx_using_subcommand store" -l include-assistant -d 'Include assistant messages (legacy flag; now default)'
complete -c aicx -n "__fish_aicx_using_subcommand store" -l no-noise-filter -d 'Disable structural-noise filter (line-numbered grep matches, tool echoes, stray YAML delimiters). Default: filter is ON. Use this for debugging or when raw upstream content must be preserved verbatim in the chunk text'
complete -c aicx -n "__fish_aicx_using_subcommand store" -s v -l verbose -d 'Verbose diagnostics: echo per-file extractor warnings to stderr'
complete -c aicx -n "__fish_aicx_using_subcommand store" -s h -l help -d 'Print help (see more with \'--help\')'
...
complete -c aicx -n "__fish_aicx_using_subcommand refs" -s p -l project -d 'Strict project filter: `owner/repo`, `/repo` (cross-org repo name), `owner/` (org wildcard), or `name` (matches org OR repo). Substring matching is intentionally disabled — `-p vista` no longer leaks into `vista-portal`/`vista-datasets`' -r
...
complete -c aicx -n "__fish_aicx_using_subcommand state" -s p -l project -d 'Project filter (applies to --info as well as --reset). Supports the standard shapes: `-p owner/repo`, `-p owner/`, `-p /repo`, or a bare `-p name` (cross-org)' -r
...
complete -c aicx -n "__fish_aicx_using_subcommand dashboard" -s p -l project -d 'Narrow the dashboard dataset to project/store buckets containing this string' -r
...
internal_thought\t''
tool_call\t''"
complete -c aicx -n "__fish_aicx_using_subcommand search" -l kind -d 'Filter by canonical corpus kind: conversations, plans, reports, other' -r -f -a "conversations\t''
conversation\t''
plans\t''
plan\t''
reports\t''
...
complete -c aicx -n "__fish_aicx_using_subcommand index; and not __fish_seen_subcommand_from status derive help" -s p -l project -d 'Project filter. Omit to index the global `_all` bucket' -r
complete -c aicx -n "__fish_aicx_using_subcommand index; and not __fish_seen_subcommand_from status derive help" -l sample -d 'Stop after sampling this many chunks (0 = scan all)' -r
...
complete -c aicx -n "__fish_aicx_using_subcommand index; and not __fish_seen_subcommand_from status derive help" -l full-rescan -d 'Force a full re-embed of every chunk. Default is incremental: walk only sidecars newer than the existing index\'s `header.generated_at` and append. Use this flag after embedder model changes or when the committed index is suspect'
...
complete -c aicx -n "__fish_aicx_using_subcommand config; and not __fish_seen_subcommand_from init show help" -s v -l verbose -d 'Verbose diagnostics: echo per-file extractor warnings to stderr'
complete -c aicx -n "__fish_aicx_using_subcommand config; and not __fish_seen_subcommand_from init show help" -s h -l help -d 'Print help (see more with \'--help\')'
complete -c aicx -n "__fish_aicx_using_subcommand config; and not __fish_seen_subcommand_from init show help" -f -a "init" -d 'Write a default `~/.aicx/config.toml` with cloud-embedder pre-selected. Bails if the file exists unless `--force`'
complete -c aicx -n "__fish_aicx_using_subcommand config; and not __fish_seen_subcommand_from init show help" -f -a "show" -d 'Display the resolved embedder configuration after merging env, `embedder.toml`, `config.toml`, and built-in defaults'
complete -c aicx -n "__fish_aicx_using_subcommand config; and not __fish_seen_subcommand_from init show help" -f -a "help" -d 'Print this message or the help of the given subcommand(s)'
...
complete -c aicx -n "__fish_aicx_using_subcommand migrate-intent-schema" -s p -l project -d 'Strict project filter: `owner/repo`, `/repo` (cross-org repo name), `owner/` (org wildcard), or `name` (matches org OR repo). Omit to scan the whole store. Substring matching is intentionally disabled' -r
...
complete -c aicx -n "__fish_aicx_using_subcommand doctor" -l apply -d 'With --prune-empty-bodies, move empty-body chunks into recoverable quarantine'
...
complete -c aicx -n "__fish_aicx_using_subcommand help; and not __fish_seen_subcommand_from completions claude codex all extract conversations store ingest list sources sessions claims results clarify wizard refs state dashboard reports corpus reports-extractor dashboard-serve intents tail serve init search eval index config read steer migrate migrate-intent-schema doctor health warmup help" -f -a "completions" -d 'Generate shell completions for the canonical CLI grammar'
complete -c aicx -n "__fish_aicx_using_subcommand help; and not __fish_seen_subcommand_from completions claude codex all extract conversations store ingest list sources sessions claims results clarify wizard refs state dashboard reports corpus reports-extractor dashboard-serve intents tail serve init search eval index config read steer migrate migrate-intent-schema doctor health warmup help" -f -a "claude" -d 'Extract and store Agents\' sessions into the canonical corpus (canonical corpus extraction)'
complete -c aicx -n "__fish_aicx_using_subcommand help; and not __fish_seen_subcommand_from completions claude codex all extract conversations store ingest list sources sessions claims results clarify wizard refs state dashboard reports corpus reports-extractor dashboard-serve intents tail serve init search eval index config read steer migrate migrate-intent-schema doctor health warmup help" -f -a "codex" -d 'Extract and store Codex sessions into the canonical corpus'
complete -c aicx -n "__fish_aicx_using_subcommand help; and not __fish_seen_subcommand_from completions claude codex all extract conversations store ingest list sources sessions claims results clarify wizard refs state dashboard reports corpus reports-extractor dashboard-serve intents tail serve init search eval index config read steer migrate migrate-intent-schema doctor health warmup help" -f -a "all" -d 'Extract and store from all agents (Claude + Codex + Gemini + Junie + Codescribe) into the canonical corpus'
...
complete -c aicx -n "__fish_aicx_using_subcommand help; and not __fish_seen_subcommand_from completions claude codex all extract conversations store ingest list sources sessions claims results clarify wizard refs state dashboard reports corpus reports-extractor dashboard-serve intents tail serve init search eval index config read steer migrate migrate-intent-schema doctor health warmup help" -f -a "store" -d 'Build the canonical corpus in from local agents\' session files'
...
complete -c aicx -n "__fish_aicx_using_subcommand help; and not __fish_seen_subcommand_from completions claude codex all extract conversations store ingest list sources sessions claims results clarify wizard refs state dashboard reports corpus reports-extractor dashboard-serve intents tail serve init search eval index config read steer migrate migrate-intent-schema doctor health warmup help" -f -a "search" -d 'Search the canonical corpus. Semantic by default; automatic filesystem-fuzzy fallback when semantic search is unavailable'
complete -c aicx -n "__fish_aicx_using_subcommand help; and not __fish_seen_subcommand_from completions claude codex all extract conversations store ingest list sources sessions claims results clarify wizard refs state dashboard reports corpus reports-extractor dashboard-serve intents tail serve init search eval index config read steer migrate migrate-intent-schema doctor health warmup help" -f -a "eval" -d 'Run local evaluation helpers for retrieval/search quality'
complete -c aicx -n "__fish_aicx_using_subcommand help; and not __fish_seen_subcommand_from completions claude codex all extract conversations store ingest list sources sessions claims results clarify wizard refs state dashboard reports corpus reports-extractor dashboard-serve intents tail serve init search eval index config read steer migrate migrate-intent-schema doctor health warmup help" -f -a "index" -d 'Catch up the canonical corpus, then build the semantic index. Use `--dry-run` to preview without writing'
